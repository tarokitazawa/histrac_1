# ============================================================
# Identity jump GLMM for A395 treatment
#
# Input example:
#   combined.obj.ls_A395$Dam_bb
#
# Comparison type:
# - Treatment vs control within HisTrac
#
# Biological question:
# - Does A395 treatment increase identity-jump frequency relative to control?
#
# Analytical premise:
...
# - Both control and A395 groups belong to the HisTrac framework.
#
# Group definition:
# - Control:
#     ctrlrep1, ctrlrep2
# - Treatment:
#     A395rep1, A395rep2, A395rep3
#
# Model:
# - Binomial GLMM
# - Formula:
...
# - Estimated jump probabilities for control and A395
# - Odds ratio for A395 vs control
# ============================================================
...
# Input
# --------------------
combined.obj.ls_A395 <- readRDS("path_to_A395/A395_ctrl_A395.rds")
obj <- combined.obj.ls_A395$Dam_bb
...
treat_ids   <- c("A395rep1", "A395rep2", "A395rep3")

# --------------------
# Output files
# --------------------
out_csv_rates <- "IdentityJump_rates_A395.csv"
out_csv_glmm  <- "IdentityJump_GLMM_A395.csv"
...
df <- meta %>%
  ...
  mutate(
    group = case_when(
      ...
      id %in% treat_ids   ~ "A395",
      TRUE ~ NA_character_
    )
  ) %>%
  filter(!is.na(group)) %>%
  mutate(
    group = factor(group, levels = c("control", "A395")),
...
z <- s$z.ratio[1]
p_one <- pnorm(z, lower.tail = FALSE)  # one-sided: A395 > control

logOR <- s$estimate[1]
res_df <- data.frame(
  dataset = "A395",
  p_jump_control = prob_df$prob[prob_df$group == "control"][1],
  p_jump_A395    = prob_df$prob[prob_df$group == "A395"][1],
  logOR_A395_vs_control = logOR,
  OR_A395_vs_control    = exp(logOR),
